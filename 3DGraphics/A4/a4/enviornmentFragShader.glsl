...
layout(binding = 1) uniform samplerCube s;
...
{	vec3 L = normalize(varyingLightDir);
	...
	vec4 texColor = texture(s, r);
	vec3 ambient = (globalAmbient + light.ambient).xyz * material.ambient.xyz;
	fragColor = vec4(texColor.rgb * ambient, texColor.a/2);
	...
	if (notInShadow  == 1.0) {
		...
		fragColor = vec4(resultColor, texColor.a/2);
	}
}
