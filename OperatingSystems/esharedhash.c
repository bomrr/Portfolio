...
__thread int isPoolInit = 0;
__thread char *poolEnd = NULL;
...
void *umalloc_fast(size_t size) {
    ...
    if (!isPoolInit) {
        fprintf(stderr, "Warning: Thread pool not created\n");
    }

    char *end = pool_start + pool_size;
    if (pool_current + size <= end) {
        void *ptr = pool_current;
        pool_current += size;
        return ptr;
    }

    fprintf(stderr, "Thread pool exhausted.");
    abort();
...
void ufree(void *ptr) {
    if (use_multiprocess) {
        ...
        // Don't need to free memory for threading
        (void)ptr;
    }
...
/**
 * Each thread now opens and reads the file on its own,
 * giving it its own pointer to the file rather than
 * relying on a global one.
 */
void *worker_thread(void *arg) {
    thread_arg_t *targ = (thread_arg_t *)arg;
    thread_pool_init(targ->thread_id, targ->num_threads);
    unsigned long threadHash = 0;

    // Read the file block
    FILE *fp = fopen(targ->filename, "rb");
    if (!fp) {
        perror("fopen");
        return NULL;
    }
    ...
    unsigned char buf[BLOCK_SIZE];
    long offset = (long)targ->startBlock * BLOCK_SIZE;
    ...
    // Read through the thread's assigned blocks
    for (int i = targ->startBlock; i < targ->endBlock; i++) {
        size_t n = fread(buf, 1, BLOCK_SIZE, fp);
        if (n == 0) {
            break;
        }
        ...
        unsigned long h = process_block(buf, n);
        threadHash = (threadHash + h) % LARGE_PRIME;
        ...
        print_intermediate(i, h, 0);
    }

    fclose(fp);
    targ->results = threadHash;
    return NULL;
}

/**
 * Follow the same general steps as run_multi, but with pthread_create instead of fork()
 */
int run_threads(const char *filename) {
    ...
    // How many threads to run, don't make more threads than there are blocks
    int maxThreads = 32;
    if (maxThreads > totalBlocks) {
        maxThreads = totalBlocks;
    }

    pthread_t threads[maxThreads];
    thread_arg_t args[maxThreads];
    int blocksPerThread = totalBlocks / maxThreads;
    int extraBlocks = totalBlocks % maxThreads;
    int nextBlock = 0;

    for (int i = 0; i < maxThreads; i++) {
        // Give each thread its number of blocks plus one from extra blocks
        int count = blocksPerThread + (i < extraBlocks ? 1 : 0);

        args[i].thread_id = i;
        args[i].num_threads = maxThreads;
        args[i].filename = filename;
        args[i].startBlock = nextBlock;
        args[i].endBlock = nextBlock + count;
        args[i].results = 0;

        nextBlock += count;

        int threadReturn = pthread_create(&threads[i], NULL, worker_thread, &args[i]);
        if (threadReturn != 0) {
            fprintf(stderr, "A thread failed to return 1.");
            for (int j = 0; j < i; j++) {
    ...
    unsigned long final_hash = 0;
    for (int i = 0; i < maxThreads; i++) {
        //print_intermediate(i, current, 0);
        final_hash = (final_hash + args[i].results) % LARGE_PRIME;
    }

    //ufree(results);
    print_final(final_hash);
    //fprintf(stdout, "Lock count: %i\n", lock_count);
    return 0;
}