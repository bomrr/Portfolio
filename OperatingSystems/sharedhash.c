...
// Changed free_list_ptr as done so in the instructions
 ...
 */
// Changed mLock as stated in the instructions
...
int run_threads(const char *filename) {
    ...
    unsigned long results[maxThreads];
    unsigned char *blockBuffers[maxThreads];

    // Create a space for each thread to be reused
    for (int i = 0; i < maxThreads; i++) {
        blockBuffers[i] = umalloc(BLOCK_SIZE);

        // Error check
        if (!blockBuffers[i]) {
            fprintf(stderr, "malloc failed for block buffer %d\n", i);
    ...
    // Phase 1: Create one thread for every block
    while (!feof(fp)) {
        size_t n = fread(buf, 1, BLOCK_SIZE, fp);
        if (n == 0) break;

        // If we've reached the max thread threshold wait for them to finish
        if (activeThreads == maxThreads) {
            for (int i = 0; i < activeThreads; i++) {
                pthread_join(threads[i], NULL);
                int block_id = nextId + i;
                print_intermediate(block_id, results[i], 0);
                final_hash = (final_hash + results[i]) % LARGE_PRIME;
            }
            nextId += activeThreads;
            activeThreads = 0;
        }

        // Create the shared memory block
        unsigned char *block = blockBuffers[activeThreads];
        if (!block) {
            fprintf(stderr, "umalloc failed for block %d\n", num_blocks);
            for (int i = 0; i < activeThreads; i++) {
                pthread_join(threads[i], NULL);
        ...
        args[activeThreads].block_id = activeThreads;
        args[activeThreads].block_buf = block;
        args[activeThreads].block_len = n;
        args[activeThreads].results = results;

        int threadReturn = pthread_create(&threads[activeThreads], NULL, worker_thread, &args[activeThreads]);
        if (threadReturn != 0) {
